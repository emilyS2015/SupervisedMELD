...
class MELD:
    def __init__(self, Y, Yt, k, vec, Phi = np.array([])):
        """
        Yt: the type of y_j
        ...
        for j in range(p):
            if Yt[j] == 0:
                ...
                if j in np.array([4,5,8,9,10,11,12,13,14,15,22,23,24,25,26,27,
                                  30,31,32,34,35,47,48]):
                    self.d[j]=3
                elif j in np.array([0,3,43,44,45]):
                    self.d[j]=2
                elif j in np.array([2,6,29,33,36,37,38,39,40,41,42]):
                    self.d[j]=4
                elif j in np.array([1,7,16,19,20,21,28,46]):
                    self.d[j]=5
                elif j in np.array([17,18,51]):
                    self.d[j]=6
                elif j in np.array([49]):
                    self.d[j]=7
                else:
                    self.d[j]=8            
        ...
        self.update=np.zeros(len(vec))
        
        for l in range(len(vec)):
            self.update[l]=self.d[vec[l]]
        
        self.d=np.array(self.update)
